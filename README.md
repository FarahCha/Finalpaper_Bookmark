...
This project shows Bookmark program using TDD.
...
## TestCases
### 1. Test Case
Involves defining several bookmarks, but all with different tags, and then filtering them by one keyword. Only one result should be returned.

### 2. Test Case
Filter for a keyword that does not exist. The result of the searchBookmarksByKeyword method should be empty.

### 3. Test Case
Has more than one bookmark with the same tag. As a result should find all bookmarks.

### 4. Test Case
Search for bookmarks while none are declared.